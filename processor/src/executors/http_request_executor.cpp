...
class HttpRequestExecutor : public BlockExecutor {
 public:
  HttpRequestExecutor() = default;
  ...
  StepResult execute(const StepRequest& req) override {
    ...
    StepResult res{};
    // Stub: perform HTTP request according to inputs: url, method, headers, body
    // For initial skeleton, return ok with echo outputs
    res.status = StatusCode::ok;
    res.outputs = req.inputs;
    res.retries_used = 0;
    res.error = "";
